...
core = vs.core
core.std.LoadPlugin(r"/usr/lib/vapoursynth/libsvpflow1_vs64.so")
core.std.LoadPlugin(r"/usr/lib/vapoursynth/libsvpflow2_vs64.so")
clip=video_in
fps = container_fps if container_fps > 0.1 else 23.976
##自适应分辨率，请自己将显示器分辨率设置好
screen_width = 1920  # 设置显示器的宽度
screen_height = 1080  # 设置显示器的高度
...
if container_fps > 77 :
	...
	smoothfps_params = "{rate:{num:2,den:2,abs:false},algo:13,gpuid:0,scene:{},mask:{area:75,cover:75}}"
else :
	smoothfps_params = "{rate:{num:2,den:1,abs:false},algo:13,gpuid:0,scene:{},mask:{area:75,cover:75}}"
