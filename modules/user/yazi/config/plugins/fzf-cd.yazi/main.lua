-- This plugin is mostly copied from yazi's builtin fzf plugin (Copyright (c) 2023 - sxyazi), except for the inclusion of fd in run_with and removing some logic for handling individual files instead of directories
local M = {}
...
function M:entry()
  ...
  if #urls == 1 then
    ...
    -- don't need to handle file results, only dirs
...
function M.run_with(cwd, selected)
  -- change from builtin: prepend fd to filter only directories
  local fd_output, err = Command("fd")
      :arg("-t")
      :arg("d")
      :output()

  if not fd_output then
    return nil, Err("Cannot read `fd` output, error: %s", err)
  end
  ...
  if #selected > 0 or fd_output.stdout ~= "" then
    child:flush()
  end
